...
object DTSCreateItemOnObject(string sItemTemplate, object oTarget)
{
    ...
    //}
     ...
     // Septire - We do not want to spawn default NWN loot. So we will remove it from here.
    // Instead, we want to log this incident.
    // object oItem = CreateItemOnObject(sItemTemplate, oTarget, nStackSize);
    string sLogString = "Container tried to spawn base NWN loot. Name: " + GetName(oTarget) + " Tag: " + GetTag(oTarget) + " Item: " + sItemTemplate;
    Log(GetName(GetArea(oTarget)), sLogString);

    // Put some gold in the chest.
    oItem = CreateItemOnObject("nw_it_gold001", oTarget, d100());
