...
void main()
{
    ...
    effect eLink = EffectLinkEffects(EffectVisualEffect(VFX_DUR_PROTECTION_GOOD_MINOR), EffectVisualEffect(VFX_DUR_CESSATE_POSITIVE));

    // Defenses against Evil
    eLink = EffectLinkEffects(eLink, VersusAlignmentEffect(EffectACIncrease(2, AC_DEFLECTION_BONUS), ALIGNMENT_ALL, ALIGNMENT_EVIL));
    eLink = EffectLinkEffects(eLink, VersusAlignmentEffect(EffectSavingThrowIncrease(SAVING_THROW_ALL, 2), ALIGNMENT_ALL, ALIGNMENT_EVIL));
    eLink = EffectLinkEffects(eLink, VersusAlignmentEffect(EffectSavingThrowIncrease(SAVING_THROW_WILL, 4), ALIGNMENT_ALL, ALIGNMENT_EVIL));
