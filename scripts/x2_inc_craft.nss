...
const string  X2_CI_BREWPOTION_NEWITEM_RESREF = "mi_it_pcpotion";       // ResRef for new potion item
...
// Craft Wondrous related constants
...
int CICraftCheckBrewPotion(object oSpellTarget, object oCaster)
{
    ...
    float nExperienceCost = (GetLocalInt(GetModule(), "STATIC_LEVEL")) ? 0.0f : 0.04  * nCost; // xp = 1/25 of gp value
    ...
    if (GetGold(oCaster) < nGoldCost)
    {
        ...
        SendMessageToPC(oCaster, "You need " + IntToString(nGoldCost) + " gold.");
        return TRUE;
    }
...
int CICraftCheckScribeScroll(object oSpellTarget, object oCaster)
{
    ...
    float fExperienceCost = (GetLocalInt(GetModule(), "STATIC_LEVEL")) ? 0.0 : 0.04 * nCost;
    int nGoldCost = FloatToInt(IntToFloat(nCost) * 0.75);

    // -------------------------------------------------------------------------
    // Dunshine, handle Arelith main server specific overrides here (i.e. raise dead and resurrection requiring piety)
    // -------------------------------------------------------------------------
    float fPietyCost;
    ...
    if (!GetLocalInt(GetModule(), "STATIC_LEVEL")) {
      switch (nID) {
        case SPELL_RAISE_DEAD: {
          // reqs 10k gold, 25% piety, 100xp
          fPietyCost = -25.0;
          fExperienceCost = 100.0f;
          nGoldCost = 10000;
          break;
        }
        case SPELL_RESURRECTION: {
          // reqs 20k gold, 50% piety, 200xp
          fPietyCost = -50.0;
          fExperienceCost = 200.0f;
          nGoldCost = 20000;
          break;
        }
      }
    ...
    // -------------------------------------------------------------------------
    // Arelith: check for sufficient Piety to cast spell
    // -------------------------------------------------------------------------
    if (fPietyCost < 0.0f) {
      float fPiety = gsWOGetPiety(oCaster);

      // check piety (do not allow below zero)
      if ((fPiety + fPietyCost) < 0.0f) {
         FloatingTextStringOnCreature("Item Creation Failed - Not enough Piety", oCaster);
         return TRUE;
      }      
    }       
    ...
    if (GetIsObjectValid(oScroll))
    {
        ...
        TakeGoldFromCreature(nGoldCost, oCaster, TRUE);
        gsXPGiveExperience(oCaster, -FloatToInt(fExperienceCost));

        // return the XP cost as adventure XP
        gvd_AdventuringXP_GiveXP(oCaster, FloatToInt(fExperienceCost), "Crafting");

        // Dunshine: handle Arelith specific diety cost here
        if (fPietyCost < 0.0f) {
...
int CICraftCheckCraftWand(object oSpellTarget, object oCaster)
{
    ...
    float nExperienceCost = (GetLocalInt(GetModule(), "STATIC_LEVEL")) ? 0.0 : 0.04 * nCost;
    int nGoldCost = FloatToInt(IntToFloat(nCost) * 0.66);
...
int CICraftCheckCraftWondrous(object oSpellTarget, object oCaster)
{
    ...
    // -------------------------------------------------------------------------
    // check if PC has learnt the Craft Wondrous feat
    // -------------------------------------------------------------------------
    if (GetLocalInt(oHide, "CRAFT_WONDROUS_ITEM") != TRUE)
    {
      FloatingTextStrRefOnCreature(40487, oCaster); // Item Creation Failed - Don't know how to create that type of item
      return TRUE; // tried item creation but do not know how to do it
    }
    ...
    // -------------------------------------------------------------------------
    // Check if item is already imbued.
    // -------------------------------------------------------------------------
     if (GetItemHasItemProperty(oSpellTarget, ITEM_PROPERTY_CAST_SPELL))
     {
       FloatingTextStringOnCreature("Item creation failed: item is already imbued.", oCaster);
       return TRUE;
     }

    // -------------------------------------------------------------------------
    // Check if the spell is allowed to be used with Craft Wand
    // Note - we use the same set of restrictions for Craft Wondrous.
    // -------------------------------------------------------------------------
    if (CIGetIsSpellRestrictedFromCraftFeat(nSpellID, X2_CI_CRAFTWAND_FEAT_ID))
    {
        FloatingTextStrRefOnCreature(83452, oCaster); // can not be used with this feat
        return TRUE;
    }
    ...
    // -------------------------------------------------------------------------
    // check if spell is below maxlevel for wands and wondrous items.
    // -------------------------------------------------------------------------
    if (nLevel > X2_CI_CRAFTWAND_MAXLEVEL)
    {
        FloatingTextStrRefOnCreature(83623, oCaster);
    ...
    // XP/GP Cost Calculation
    // -------------------------------------------------------------------------
    int nCost = CIGetCraftGPCost( nLevel, X2_CI_CRAFTWONDROUS_COSTMODIFIER);
    float nExperienceCost = (GetLocalInt(GetModule(), "STATIC_LEVEL")) ? 0.0 : 0.04 * nCost;
    int nGoldCost = FloatToInt(IntToFloat(nCost));
    ...
    // -------------------------------------------------------------------------
    // One use per day.
    // TODO - allow multiple uses per day.
    // -------------------------------------------------------------------------
    itemproperty ipSpell = ItemPropertyCastSpell(nPropID, IP_CONST_CASTSPELL_NUMUSES_1_USE_PER_DAY);

    // -------------------------------------------------------------------------
    // FL only - make Alchemy craft check vs item value.
    // -------------------------------------------------------------------------
    if (GetLocalInt(GetModule(), "STATIC_LEVEL"))
    {
      // Make an Alchemy skill check.
      nGoldCost = FloatToInt(IntToFloat(nGoldCost) * gsCRGetCraftingCostMultiplier(oCaster, oSpellTarget, ipSpell));

      // Impose a min cost to avoid abusing merchants.
      if (nGoldCost < 300) nGoldCost = 300;

      int nChance = 0;  // Auto success for now.
	  
      int nImpossible = FALSE;
      /*
      int nBaseItemValue = FloatToInt(gsCRGetCraftingCostMultiplier(oCaster, oSpellTarget, ipSpell)
                              * IntToFloat(gsCMGetItemValue(oSpellTarget)));

      nChance = (nBaseItemValue + nGoldCost) * 1/100;
      ...
      */
      ...
      if (Random(100) < nChance)
      {
        ...
        if (!nImpossible &&
            gsWOGetDeityAspect(oCaster) & ASPECT_MAGIC &&
            d2() == 2 &&
