...
void main()
{
   //return; // Arelith edit to disable mounting.

   object oPC=OBJECT_SELF;
   object oTarget=GetSpellTargetObject();

   if (GetIsDM(oPC) || GetIsDMPossessed(oPC) || GetIsObjectValid(GetItemPossessedBy(oPC, "gvd_dm_horsetemp")))
   {
		//dm immune to horse antics
   }
   else if ( kiGetRideRank(oPC) >= 6 )
   {
        //SetLocalInt(gsPCGetCreatureHide(oPC), "Mounted", 1);
        kiThrowRiderCheck(oPC, oTarget);
   }
   else
   {
     //AssignCommand(oTarget, ActionSpeakString("Neigh!"));
     FloatingTextStringOnCreature("You need at least 6 Ride skill to ride horses.", oPC);
	 kiThrowRiderCheck(oPC, oTarget);
     return;
   }
   ...
   {
     //AssignCommand(oTarget, ActionSpeakString("Neigh!"));
     FloatingTextStringOnCreature("You can only ride horses above ground and outside.", oPC);
     return;
   }
       ...
       { // Mount Menu Selection
           ...
           { // Henchman selected - not mountable
               ...
               { // not valid
                   ...
                   if (GetLevelByClass(CLASS_TYPE_PALADIN,oTarget))
