...
{
    // Peppermint, 2-5-2016; Modified this so that negative values can be passed
    // to curtail behaviors.
...
int chooseTactics(object oIntruder)
{
    // Don't attempt to heal ourself if we have spell failure. This is a workaround
    // to let us shut off healing for certain henchmen.
    if(!GetHasEffect(EFFECT_TYPE_SPELL_FAILURE, OBJECT_SELF))
    {
   ...
   // **************************************
    ...
    if (GetHasFeatEffect(FEAT_BARBARIAN_RAGE)== TRUE || GetShouldNotCastSpellsBecauseofArmor(OBJECT_SELF, nClass) == TRUE
        || GetLocalInt(OBJECT_SELF, "X2_L_STOPCASTING") == 10 || GetHasEffect(EFFECT_TYPE_SPELL_FAILURE, OBJECT_SELF))
    {
        nMagic = 0;
    }
...
void DetermineCombatRound(object oIntruder = OBJECT_INVALID, int nAI_Difficulty = 10)
{
    // Peppermint, 1-15-2016; Variables used to determine associate targeting.
    int bAcquireNewTargets = TRUE;
    ...
    // If we're an associate and we have been given the go ahead to attack, then focus on the target
    // assigned to us if it is valid.
    if(bIsPCAssociate && bAssociateAttack && GetIsObjectValid(oAssociateAttackTarget) && !GetIsDead(oAssociateAttackTarget))
    {
        oIntruder = oAssociateAttackTarget;
        bAcquireNewTargets = FALSE;
    }
