...
*/
    ...
    effect eLink = EffectLinkEffects(EffectVisualEffect(VFX_DUR_PROTECTION_EVIL_MAJOR), EffectVisualEffect(VFX_DUR_CESSATE_POSITIVE));

    // Defenses against Good
    eLink = EffectLinkEffects(eLink, VersusAlignmentEffect(EffectACIncrease(4, AC_DEFLECTION_BONUS), ALIGNMENT_ALL, ALIGNMENT_GOOD));
    eLink = EffectLinkEffects(eLink, VersusAlignmentEffect(EffectSavingThrowIncrease(SAVING_THROW_WILL, 6), ALIGNMENT_ALL, ALIGNMENT_GOOD));
    ...
    eLink = EffectLinkEffects(eLink, VersusAlignmentEffect(EffectSavingThrowIncrease(SAVING_THROW_WILL, 3), ALIGNMENT_ALL, ALIGNMENT_NEUTRAL));

    // SR
    eLink = EffectLinkEffects(eLink, EffectSpellResistanceIncrease(5 + nCasterLevel));

    // Damage Shield
    eLink = EffectLinkEffects(eLink, EffectDamageShield(DAMAGE_BONUS_6, DAMAGE_BONUS_1d8, DAMAGE_TYPE_NEGATIVE));
