...
void main()
{
    ...
    // Arelith edit to disable mounts.
    int bNoMounts = !GetLocalInt(gsPCGetCreatureHide(oPC), "MAY_RIDE_HORSE") && !GetIsObjectValid(GetItemPossessedBy(oPC, "gvd_dm_horsetemp"));

    // Paladins with the Horseriding Award get infinite uses of this feat.
    if (GetLocalInt(gsPCGetCreatureHide(oPC), "MAY_RIDE_HORSE")) {
        IncrementRemainingFeatUses(oPC, FEAT_PALADIN_SUMMON_MOUNT);
    } else {
        SendMessageToPC(oPC, "Summon Mount is disabled, except for Paladins with the legacy Ride reward.");
    }
