...
void main()
{
	...
	if (sTag == "RESEARCH_JOURNAL")
	{
	  // Either this was used on a research note to add it, or used to read notes (no target or self target).
	  if (oTarget == oActivator || !GetIsObjectValid(oTarget))
	  {
          SetLocalString(oActivator, "dialog", "zdlg_res_journal");
		  SetLocalObject(oActivator, "RESEARCH_JOURNAL", oItem);
          AssignCommand(oActivator, ActionStartConversation(oActivator, "zdlg_converse", TRUE, FALSE));
	  }
	  else if (GetStringLeft(GetTag(oTarget), 14) == "research_note_")
	  {
	    if (GetLocalString(oItem, GetTag(oTarget)) == "")
		{
			int nNote = StringToInt(GetStringRight(GetTag(oTarget), GetStringLength(GetTag(oTarget)) - 14));
			
			int nMaxNote = GetLocalInt(oItem, "MAX_NOTE");
			if (nNote > nMaxNote) SetLocalInt(oItem, "MAX_NOTE", nNote);
			SetLocalString(oItem, GetTag(oTarget), GetName(oTarget));
			gsCMReduceItem(oTarget);
		}
		else
		{
			SendMessageToPC(oActivator, "That note is already in this journal!");
		}
	  }
	  else
	  {
...
void ExecuteCustomItemBehaviors()
{
    ...
    //custom: item tag must have prefix 'I_' or 'OBJ_' and script named like item tag must be provided
    else if (GetStringLeft(sTag, 2) == "I_" || GetStringLeft(sTag, 4) == "OBJ_")
    {
