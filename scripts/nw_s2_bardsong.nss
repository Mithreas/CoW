...
void main()
{
    ...
    // Master bard (5% item)
    if (GetIsObjectValid(GetItemPossessedBy(OBJECT_SELF, "mi_master_harp")))
    {
      nMultiplier = 2;
    }
    ...
    eAttack = EffectAttackIncrease(nAttack * nMultiplier);

    nDamage = nDamage * nMultiplier;
    if (nDamage > 5) nDamage+=10; // DAMAGE_BONUS_* constants have odd numbering
    ...
    if(nWill > 0)
    {
        eWill = EffectSavingThrowIncrease(SAVING_THROW_WILL, nWill * nMultiplier);
        eLink = EffectLinkEffects(eLink, eWill);
    }
    if(nFort > 0)
    {
        eFort = EffectSavingThrowIncrease(SAVING_THROW_FORT, nFort * nMultiplier);
        eLink = EffectLinkEffects(eLink, eFort);
    }
    if(nReflex > 0)
    {
        eReflex = EffectSavingThrowIncrease(SAVING_THROW_REFLEX, nReflex * nMultiplier);
...
//        eLink = EffectLinkEffects(eLink, eHP);
    ...
    if(nAC > 0)
    {
        eAC = EffectACIncrease(nAC * nMultiplier, AC_DODGE_BONUS);
        eLink = EffectLinkEffects(eLink, eAC);
    }
    if(nSkill > 0)
    {
        eSkill = EffectSkillIncrease(SKILL_ALL_SKILLS, nSkill * nMultiplier);
    ...
    while(GetIsObjectValid(oTarget))
    {
        ...
        if(!GetHasFeatEffect(FEAT_BARD_SONGS, oTarget))
        {
             ...
             {
			    if (bCuresLycanthropy && GetLocalInt(oTarget, "MI_AWIA"))
				{
    ...
    {
      chatSpeakString(OBJECT_SELF, NWNX_CHAT_CHANNEL_PLAYER_TALK, "*appears to be singing* ~Brains urrgh... brains braaains!~");
      return;
    }
}
