...
/*
    ...
    Arelith Specific:  CoT stacks for turn attempts
*/
//:://////////////////////////////////////////////
...
void main()
{
    ...
    if (fbZGetIsZombie(OBJECT_SELF))
    {
      ...
      {
        FloatingTextStringOnCreature("You should probably rethink that. You don't want to risk turning yourself, do you?", OBJECT_SELF, FALSE);
      }
      return;
    }
    ...
    int nCoTlevel = GetLevelByClass(CLASS_TYPE_DIVINECHAMPION);
    int nHarperLevel = 0;

    if (GetLocalInt(gsPCGetCreatureHide(), VAR_HARPER) == MI_CL_HARPER_PRIEST)
    {
      nHarperLevel = GetLevelByClass(CLASS_TYPE_HARPER);
      if (nHarperLevel > 1) nHarperLevel += 3; // Bonus to Turn Undead at level 2.
    }

    int nTotalLevel =  GetHitDice(OBJECT_SELF);

    int nTurnLevel = nClericLevel + nHarperLevel + nCoTlevel;
    int nClassLevel = nClericLevel + nHarperLevel + nCoTlevel;
    ...
    while(GetIsObjectValid(oTarget) && nHDCount < nTurnHD && GetDistanceToObject(oTarget) <= 20.0)
    {
        if(!GetIsFriend(oTarget))
        {
            ...
            if(nHD <= nTurnLevel && nHD <= (nTurnHD - nHDCount))
            {
                ...
                if((nRacial == RACIAL_TYPE_UNDEAD) || (gsSUGetSubRaceByName(GetSubRace(oTarget)) == GS_SU_SPECIAL_VAMPIRE))
                {
                    bValid = TRUE;
                }
                ...
                else if ( (nRacial == RACIAL_TYPE_OUTSIDER || bPlayerOutsider) && (nGoodOrEvilDomain+nPlanar > 0) )
                {
                    bValid = TRUE;
                }
                ...
                if( bValid == TRUE)
                {
                    ...
                    if ( nPlanar > 0 && (nRacial == RACIAL_TYPE_OUTSIDER || bPlayerOutsider) )
                    {
                    ...
                    else
                    {
                        ...
                        //::  Player Outsiders
                        if (bPlayerOutsider) {
                            ApplyFear(oTarget, 10,  RoundsToSeconds(nClassLevel + 5));
                        }
                        //::  Default
                        else {
                            //Fire cast spell at event for the specified target
