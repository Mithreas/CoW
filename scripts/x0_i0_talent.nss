...
int TalentMeleeAttack(object oIntruder = OBJECT_INVALID)
{
    ...
    if(nDiff < 10)
    {
        ...
        tUse = GetCreatureTalent(TALENT_CATEGORY_HARMFUL_MELEE,
                                     GetCRMax());
        //if (GetIsObjectValid(GetMaster(OBJECT_SELF))) SpeakString("Melee Talent Valid = "+ IntToString(GetIsTalentValid(tUse)) + ", Feat ID = " + IntToString(GetIdFromTalent(tUse)));
    ...
    {
	    //if (GetIsObjectValid(GetMaster(OBJECT_SELF))) SpeakString("Just doing standard attack.");
        //MyPrintString("TalentMeleeAttack Successful Exit");
