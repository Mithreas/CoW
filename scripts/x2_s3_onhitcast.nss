...
void main()
{
   ...
   if (GetIsObjectValid(oItem))
   {
     ...
     {
        i++;
        sScript = GetLocalString(oItem, "RUN_ON_HIT_" + IntToString(i));
        if(ExecuteScriptAndReturnInt(sScript, OBJECT_SELF) && i != 1)
            break;
     } while(sScript != "" && i <= 10);
     ...
     if (GetModuleSwitchValue(MODULE_SWITCH_ENABLE_TAGBASED_SCRIPTS) == TRUE)
     {
        ...
        nRet =   ExecuteScriptAndReturnInt(GetUserDefinedItemEventScriptName(oItem),OBJECT_SELF);
