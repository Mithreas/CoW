...
void DoSpellBreach(object oTarget, int nTotal, int nSR, int nSpellId = -1)
{
    ...
    if(!GetIsReactionTypeFriendly(oTarget))
    {
        ...
        while(nCnt <= NW_I0_SPELLS_MAX_BREACH && nIdx < nTotal)
        {
            ...
            nIdx = nIdx + RemoveProtections(nBreachId, oTarget, nCnt);
            nCnt++;
        }
...
int GetSpellBreachProtection(int nLastChecked)
{
    ...
    else if(nLastChecked == 14) {return SPELL_WOUNDING_WHISPERS;}
    ...
    else if(nLastChecked == 24) {return SPELL_SHADOW_SHIELD;}
    ...
    else if(nLastChecked == 27) {return SPELL_FREEDOM_OF_MOVEMENT;}
