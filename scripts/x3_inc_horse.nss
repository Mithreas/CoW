...
object HorseDismount(int bAnimate=TRUE,int bSetOwner=TRUE, int bThrown=FALSE);
...
//::Kirito - Consequences of being thrown
void kiThrowRider(object oPC);
//::Kirito - Check if rider will be thrown
void kiThrowRiderCheck(object oPC, object oHorse, int bDelay = TRUE, int bCombat = FALSE, int nOther = 0);
//::Kirito - Check Dismount from Joust
void kiCheckJoustKD(object oAttacker, object oDefender, int nAttackerMod = 0);
//::Kirito - Check Hit from Joust
void kiCheckJoustHit(object oRider1, object oRider2);
...
{ // PURPOSE: Return which appearance the rider should use when mounted
    ...
    // feedback for debugging horse mount issues    
    SendMessageToPC(oRider, "No mounted appearance for this race");
...
{ // PURPOSE: Change the movement speed based on that of the horse (wont work unless EffectMovementSpeedIncrease() bug is fixed)
    ...
    int nSpeed=kiBreedSpeed(oRider, oHorse);
    effect eMovement;
    float fSpeed;
    int nMonkEpic;
    int nMonk=GetLevelByClass(CLASS_TYPE_MONK,oRider);
	
	if (nSpeed==0) nSpeed=10;
	//if (nSpeed==0) nSpeed=HORSE_DEFAULT_SPEED_INCREASE; //::kirito - comment out when horses in
	
	//::kirito add speed changes to horses
	int nRankSpeed = kiGetRideRank(oRider, FALSE, TRUE) / 3;
	if (nRankSpeed > 20) nRankSpeed = 20; // limit attainable only for horse awards
	nSpeed += nRankSpeed; // add riderank speed increase to horse speed  - 5% per 15 ranks
...
{ // PURPOSE: This will transfer preserved values for the mounted horse to
  // oHorse from oRider and then will remove them from oRider
    ...
    { // remove horse modifiers
		...
		DelayCommand(0.25*fX3_MOUNT_MULTIPLE,ki_Wrapr_Dismount(oRider, oHorse));
...
{ // PURPOSE: Return whether oTarget is mounted
    ...
    { // valid parameter
        //if (GetSkinInt(oTarget,"bX3_IS_MOUNTED")) return TRUE; -- Arelith edit
        switch (GetRacialType(oTarget))
        {
          ...
          case RACIAL_TYPE_HUMAN:
            if (GetAppearanceType(oTarget) == HORSE_SupportGetMountedAppearance(oTarget)) return TRUE;
            break;
          default:
            // Other races cannot ride.
            return FALSE;
        }
    } // valid parameter
    return FALSE;
...
{ // PURPOSE: blocked path, death, entanglement, confusion, paralysis, stun, petrification, fear, turned, sleep, knockdown detection by Azbest
    ...
    else
    {
        ...
        { // traverse effects
            // dunshine, removed this line, since I suspect it prevents characters with a tagged effect using Horses (like Kensai), not even sure what the purpose of it is:
            // (GetEffectType(eEffect)==EFFECT_TYPE_INVALIDEFFECT&&(GetEffectDurationType(eEffect)==DURATION_TYPE_TEMPORARY||GetEffectDurationType(eEffect)==DURATION_TYPE_PERMANENT))||
            ...
            { // match
                ...
                // feedback for debugging horse mount issues    
                SendMessageToPC(oCreature, "One of the effects on your PC prevents it from mounting a horse.");
...
{ // PURPOSE: Handle the mounting of oHorse
  ...
  // 6 = destroy oHorse
	...
	//::kirito
	SetLocalInt(gsPCGetCreatureHide(oRider), "Mounted", 1);
	//::
} // HorseMount()
...
{ // PURPOSE: Dismount the horse
    ...
    location lLoc;
	//::kirito
	SetLocalInt(gsPCGetCreatureHide(oRider), "Mounted", 0);
	//::
    float fX3_MOUNT_MULTIPLE=GetLocalFloat(GetArea(oRider),"fX3_MOUNT_MULTIPLE");
    float fX3_DISMOUNT_MULTIPLE=GetLocalFloat(GetArea(oRider),"fX3_DISMOUNT_MULTIPLE");
    if (GetLocalFloat(oRider,"fX3_MOUNT_MULTIPLE")>fX3_MOUNT_MULTIPLE) fX3_MOUNT_MULTIPLE=GetLocalFloat(oRider,"fX3_MOUNT_MULTIPLE");
    ...
    { // oRider is a valid object
        ...
        { // Mounted
            ...
            { // create dismounted horse
				...
				//::Kirito-make horse run away
				if(bThrown)
				{
					//Locate the area we are in
					object oArea = GetArea(OBJECT_SELF);
					//Locate where in the are we are
					vector vOriginal = GetPosition(OBJECT_SELF);
					...
					//opposite direction
					//float fDirection = GetOppositeDirection(fOrientation);
					
					float fDistance = IntToFloat(Random(35) + 15);
					
					vector vPosition = GetChangedPosition(vOriginal, fDistance, fDirection);
					
					location lDestination = Location( oArea, vPosition, fDirection );
					DelayCommand(2.0,AssignCommand(oHorse, ActionMoveToLocation(lDestination,TRUE))); 
				}
            } // create dismounted horse
            else
            { // resref not defined
...
{ // PURPOSE: Instantly dismount oRider
	...
	DelayCommand(0.3*fX3_MOUNT_MULTIPLE,ki_Wrapr_Dismount(oRider));
...
{ // PURPOSE: Summon Paladin Mount
    ...
    if (!GetIsObjectValid(oMount)&&nLevel&&GetObjectType(oSummoner)==OBJECT_TYPE_CREATURE)
...
{ // PURPOSE: HorseChangeToDefault
	...
	DelayCommand(0.4,ki_Wrapr_Dismount(oCreature));
...
{ // Wrap
    ...
    { // make sure dismount
		...
		DelayCommand(1.0,ki_Wrapr_Dismount(oNPC));
...
////////////////////////////////////////////////////////////////////////////////
///////////////////////////    Arelith Specific    /////////////////////////////
//////////////////////// Stops Circular Dependencies ///////////////////////////
////////////////////////////////////////////////////////////////////////////////

//::Kirito - Consequences of being thrown
void kiThrowRider(object oPC)
{
    if(GetLocalInt(gsPCGetCreatureHide(oPC), "Mounted") == 1)
    {
        SetLocalInt(gsPCGetCreatureHide(oPC), "Mounted", 0);
        FloatingTextStringOnCreature("You have been thrown from your horse.", oPC);
        HorseDismount(FALSE,TRUE,TRUE);
		int nTumble = 6 - GetSkillRank(SKILL_TUMBLE, oPC, TRUE) / 5;
		
		if (GetSkillRank(SKILL_TUMBLE, oPC, FALSE) <= 35)
		{
			ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectDamage(d4(nTumble), DAMAGE_TYPE_BLUDGEONING), oPC);
			ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectKnockdown(), oPC, 3.0 + IntToFloat(nTumble));
		}
    }
}
...
{
    int nBonus;
	int nBreed = kiBreedRideBonus(oPC, oHorse);
	int nDrunk = kiDrunkRideBonus(oPC);
	int nCombat = kiCombatRideBonus(oPC, oHorse, bCombat);
	
	nBonus = nBreed + nDrunk + nCombat + nOther;
	
	if (bDelay)
	{
		if (kiGetRideRank(oPC) < 6)
		{
			DelayCommand(IntToFloat(d4()),kiThrowRider(oPC));
		}
		else if (kiGetRideRank(oPC, TRUE) + nBonus < 15)
		{
			int nCheckvalue = d20() - 5 + kiGetRideRank(oPC, FALSE) + nBonus;
			if (nCheckvalue <  11)
			{
				DelayCommand(IntToFloat(d100()*6),kiThrowRider(oPC));
			}
		}
	}
	else
	{
		...
		{
			int nCheckvalue = d20() - 5 + kiGetRideRank(oPC, TRUE) + nBonus;
			if (nCheckvalue <=  11)
			{
				kiThrowRider(oPC);
			}
		}
		else if (kiGetRideRank(oPC, TRUE) + nBonus <= 20 && d20() == 1)
		{
			kiThrowRider(oPC);
		}
	}
...
void kiCheckJoustHit(object oRider1, object oRider2)
{
	int nRider1Score = kiGetRideRank(oRider1);
	if (nRider1Score > 40) //horse award characters get a little bonus from ride ranks
	{
		nRider1Score = 40;
	} 
	int nRider1Dice = d20();
	int nRider2Score = kiGetRideRank(oRider2);
	if (nRider2Score > 40) //horse award characters get a little bonus from ride ranks
	{
		nRider2Score = 40;
	} 
	int nRider2Dice = d20();
	
	if (nRider1Dice == 20 || nRider1Score + nRider1Dice > nRider2Score + nRider2Dice + 10)
	{
		kiCheckJoustKD(oRider1, oRider2, 5);
	}
	else if (nRider1Score + nRider1Dice > nRider2Score + nRider2Dice )
	{
		kiCheckJoustKD(oRider1, oRider2);
	}
	else if (nRider1Score + nRider1Dice > nRider2Score + nRider2Dice -5)
	{
		kiCheckJoustKD(oRider1, oRider2, -5);
	}
	else
	...
	if (nRider2Dice == 20 || nRider2Score + nRider2Dice > nRider1Score + nRider1Dice + 10)
	{
		kiCheckJoustKD(oRider2, oRider1, 5);
	}
	else if (nRider2Score + nRider2Dice > nRider1Score + nRider1Dice )
	{
		kiCheckJoustKD(oRider2, oRider1);
	}
	else if (nRider2Score + nRider2Dice > nRider1Score + nRider1Dice -5)
	{
		kiCheckJoustKD(oRider2, oRider1, -5);
	}
	else
...
void kiCheckJoustKD(object oAttacker, object oDefender, int nAttackerMod = 0)
{
	//effectively attacker max score of 25 + STR vs Defender max score of 30 + STR, horse awards give + 5 to defender
	
	//setup 
	int nAttackerScore = 3 + GetBaseAttackBonus(oAttacker) + GetAbilityModifier(ABILITY_STRENGTH, oAttacker);
	int nAttackerDice = d20() + nAttackerMod;
	int nDefenderScore = kiGetRideRank(oDefender);
	if (nDefenderScore > 38) //horse award characters get a little bonus from ride ranks
	{
		nDefenderScore = 38;
	} 
	nDefenderScore += GetAbilityModifier(ABILITY_STRENGTH, oAttacker);
	int nDefenderDice = d20();
	
	// do the checks
	if ((nAttackerScore + nAttackerDice > nDefenderScore + nDefenderDice && (nAttackerDice >= 20 || nDefenderDice == 1)) 
		|| (nAttackerScore + nAttackerDice > nDefenderScore + nDefenderDice + 10))
	{
		kiThrowRider(oDefender);
		//joust violent fall animation
	}
	else if (nAttackerScore + nAttackerDice > nDefenderScore + nDefenderDice)
	{
		kiThrowRider(oDefender);
		//joust fall animation
	}
	else if (nAttackerMod > 0)
	{
		//helm off
	}
	else
