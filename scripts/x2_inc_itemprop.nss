...
void  IPWildShapeCopyItemProperties(object oOld, object oNew, int bWeapon = FALSE, int bStack = FALSE);
...
// Wrapper for CopyItemAndModify that preserves description changes.
...
object gsCMCopyItemAndModify(object oItem, int nType, int nIndex, int nNewValue, int bCopyVars=FALSE);

// * Verifies validity and returns targeted or equiped melee weapon
object IPGetTargetedOrEquippedMeleeWeapon();
// * Verifies validity and returns targeted or equiped armour
...
object gsCMCopyItemAndModify(object oItem, int nType, int nIndex, int nNewValue, int bCopyVars=FALSE)
{
  ...
  object oReturn = CopyItemAndModify(oItem, nType, nIndex, nNewValue, bCopyVars);
  SetDescription(oReturn, sDescription);
  SetDroppableFlag(oReturn, nDroppable);
  return oReturn;
}
...
object IPDyeArmor(object oItem, int nColorType, int nColor)
{
        object oRet = gsCMCopyItemAndModify(oItem,ITEM_APPR_TYPE_ARMOR_COLOR,nColorType,nColor,TRUE);
...
object IPGetModifiedArmor(object oArmor, int nPart, int nMode, int bDestroyOldOnSuccess)
{
    ...
    object oNew = gsCMCopyItemAndModify(oArmor,ITEM_APPR_TYPE_ARMOR_MODEL, nPart, nNewApp,TRUE);
...
object IPGetModifiedWeapon(object oWeapon, int nPart, int nMode, int bDestroyOldOnSuccess)
{
    ...
    //SpeakString("new: " + IntToString(nNewApp));
    object oNew = gsCMCopyItemAndModify(oWeapon,ITEM_APPR_TYPE_WEAPON_MODEL, nPart, nNewApp,TRUE);
    if (oNew != OBJECT_INVALID)
    {
        if( bDestroyOldOnSuccess )
        {
...
{
    object oRet = gsCMCopyItemAndModify(oArmor,ITEM_APPR_TYPE_ARMOR_MODEL,ITEM_APPR_ARMOR_MODEL_ROBE,nRobeType+2,TRUE);
...
// ----------------------------------------------------------------------------
// Utility method to allow for stacking item properties. 
// Note - this is called within a method using GetFirstItemProperty/
// GetNextItemProperty.  So we can't use those methods here without looping.
// We therefore rely on saving off values based on analysing the old item - 
// see IPWildShapeCopyItemProperties for details.
// ----------------------------------------------------------------------------
void _AddItemProperty(itemproperty ip, object oItem)
{ 
  itemproperty newIp;
  
  if (GetItemPropertyType(ip) == ITEM_PROPERTY_ABILITY_BONUS)
  {
    int nAbility = GetItemPropertySubType(ip);
    int nBonus = GetLocalInt(oItem, "ABILITY_" + IntToString(nAbility));
    DeleteLocalInt(oItem, "ABILITY_" + IntToString(nAbility)); // Ensure never used twice.								  
								  
    newIp = ItemPropertyAbilityBonus(nAbility, GetItemPropertyCostTableValue(ip) + nBonus);

    AddItemProperty(DURATION_TYPE_PERMANENT, newIp, oItem);   
  }
  else if (GetItemPropertyType(ip) == ITEM_PROPERTY_SKILL_BONUS)
  {
    int nSkill = GetItemPropertySubType(ip);
    int nBonus = GetLocalInt(oItem, "SKILL_" + IntToString(nSkill));
    DeleteLocalInt(oItem, "SKILL_" + IntToString(nSkill)); // Ensure never used twice.								  
...
{ 
    ...
    {
	    // If we're stacking, work out whether the new item has any stat bonuses.
        itemproperty ip = GetFirstItemProperty(oNew);
		...
		if (bStack)
		{
          while (GetIsItemPropertyValid(ip))
          {
		    if (GetItemPropertyType(ip) == ITEM_PROPERTY_ABILITY_BONUS)
			{
			  // Save off the ability score bonus of the old item.
			  SetLocalInt(oNew, "ABILITY_" + IntToString(GetItemPropertySubType(ip)), 
			                      GetItemPropertyCostTableValue(ip));
			}	
			
			if (GetItemPropertyType(ip) == ITEM_PROPERTY_SKILL_BONUS)
			{
			  // Save off the skill bonus of the old item.
			  SetLocalInt(oNew, "SKILL_" + IntToString(GetItemPropertySubType(ip)), 
			                      GetItemPropertyCostTableValue(ip));
			}
			
			ip = GetNextItemProperty(oNew);
		  }		  
		}
