...
{
  object oTrap = CreateTrapAtLocation(nType, lLoc, 2.0f, "", STANDARD_FACTION_HOSTILE, "trap_xp", "trap_dispel");
...
void main()
{
  ...
  eEffect   = EffectLinkEffects(eEffect, EffectDispelMagicAll(nDisarmDC));
  ...
  if (!GetLocalInt(oTrap, "DYNAMIC_TRAP") && GetTrapOneShot(oTrap))
  {
	...
	{
      AssignCommand(GetModule(), DelayCommand(fRespawn, CreateTrapOnObject(nType, oTrap, STANDARD_FACTION_HOSTILE, "trap_xp", "trap_dispel")));	  
	}
  }										  
}