...
//:: Internal function, used in DoDirgeEffect to properly stack penalties.
void _DoDirgeStackPenalty(object oTarget, float nDuration);

void spellsInflictTouchAttack(int nDamage, int nMaxExtraDamage, int nMaximized, int vfx_impactHurt, int vfx_impactHeal, int nSpellID, int bOverrideEmpower = FALSE);
...
// * fDuration: Duration of petrification. If left at the default value, will last 1 round / level.
void DoPetrification(int nPower, object oSource, object oTarget, int nSpellID, int nFortSaveDC, float fDuration = -1.0);
...
/*
    ...
    March 2017
    Properly make Dirge penalties stack without the use of LocalVars.
    ...
    if (spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, GetAreaOfEffectCreator()))
    {
        ...
        if(!MyResistSpell(GetAreaOfEffectCreator(), oTarget))
        {
            ...
            if(!MySavingThrow(SAVING_THROW_FORT, oTarget, GetSpellSaveDC(), SAVING_THROW_ALL, GetAreaOfEffectCreator()))
            {
                ...
                //:: Reasssign this command to allow penalties to stack.
                AssignCommand(oTarget, _DoDirgeStackPenalty(oTarget, fDuration));
...
//:://////////////////////////////////////////////
/*
    1d4 damage/3 Caster Levels, plus a 24 hr slow if take damage.
*/
//:://////////////////////////////////////////////
//:: Created By:
//:: Created On:
...
void DoSpikeGrowthEffect(object oTarget)
{
    ...
    if (spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, GetAreaOfEffectCreator()))
    {
        ...
        if(!MyResistSpell(GetAreaOfEffectCreator(), oTarget, fDelay))
        {
            ...
            int nNumberOfDice = nCasterLevel / 3;
            int nDam = MaximizeOrEmpower(4, nNumberOfDice, nMetaMagic);
...
/*
    ...
    bOverrideEmpower: if set TRUE, spell will be empowered
*/
//:://////////////////////////////////////////////
//:: Created By:
//:: Created On:
...
void spellsInflictTouchAttack(int nDamage, int nMaxExtraDamage, int nMaximized, int vfx_impactHurt, int vfx_impactHeal, int nSpellID, int bOverrideEmpower = FALSE)
{
    ...
    int nExtraDamage = AR_GetCasterLevel(OBJECT_SELF); // * figure out the bonus damage
    ...
    if (nMetaMagic == METAMAGIC_EMPOWER || bOverrideEmpower)
    {
        nDamage = nDamage + (nDamage / 2);
    }
                ...
                if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay))
                {
                    for (i=1; i <= nExtraMissiles + nRemainder; i++)
                    {
                        ...
                        if(nReflexSave)
                        {
                            ...
                            {
                                nDam = AR_GetReflexAdjustedDamage(nDam, oTarget, GetSpellSaveDC(), SAVING_THROW_TYPE_ELECTRICITY);
                            }
                            else if ( nDAMAGETYPE == DAMAGE_TYPE_FIRE )
                            {
                                nDam = AR_GetReflexAdjustedDamage(nDam, oTarget, GetSpellSaveDC(), SAVING_THROW_TYPE_FIRE);
                            }
                        }
...
void DoMagicFang(int nPower, int nDamagePower)
{
    ...
    effect eDamage = SupernaturalEffect(EffectDamageIncrease(nDamage));
    effect eReduction = SupernaturalEffect(EffectDamageReduction(nPower, nDamagePower)); // * doing this because
                                                                     // * it creates a true
                                                                     // * enhancement bonus
    effect eAC = SupernaturalEffect(EffectACIncrease(nPower));
    ...
    float nDuration = 6.0*60.0*AR_GetCasterLevel(OBJECT_SELF); // * Duration 1 hour/level
     if (nMetaMagic == METAMAGIC_EXTEND)    //Duration is +100%
    {
         nDuration = nDuration * 2;
    }
...
void DoPetrification(int nPower, object oSource, object oTarget, int nSpellID, int nFortSaveDC, float fDuration = -1.0)
{
    ...
    if(!GetIsReactionTypeFriendly(oTarget))
    {
        ...
        if(fDuration <= 0.0)
        {
            fDuration = RoundsToSeconds(nPower);
        }
...
int spellsIsTarget(object oTarget, int nTargetType, object oSource)
{
    ...
    if(!GetIsPC(oSource) && nTargetType == SPELL_TARGET_STANDARDHOSTILE)
...
int spellsIsImmuneToPetrification(object oCreature)
{
    ...
    if(GetIsCrowdControlImmune(oCreature))
    {
        bImmune = TRUE;
    }

    if(GetLocalInt(oCreature, "PETRIFICATION_IMMUNE"))
    {
        bImmune = TRUE;
    }
