...
int X2PreSpellCastCode(int bCheckAntimagic = TRUE);
...
int X2CastOnItemWasAllowed(object oItem)
{
    // 2da name used explicitly to avoid circular dependencies with x2_inc_craft
...
int X2PreSpellCastCode(int bCheckAntimagic = TRUE)
{
   ...
   if (nContinue)
   {
       ...
       // X2_S_UD_SPELLSCRIPT is set to gs_spellscript
       //-----------------------------------------------------------------------
       if(!bCheckAntimagic) SetLocalInt(OBJECT_SELF, "BypassAntimagic", TRUE);
