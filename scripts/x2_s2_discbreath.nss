...
void main()
{
    ...
    if(GetIsTimelocked(OBJECT_SELF, "Dragon Breath"))
    {
        TimelockErrorMessage(OBJECT_SELF, "Dragon Breath");
        return;
    }

    int nType = GetSpellId();
    int nLevel = GetLevelByClass(37,OBJECT_SELF);// 37 = red dragon disciple
    int nDamage = nLevel * (15 + d4());
    int nSaveDC = 15 + nLevel + GetAbilityModifier(ABILITY_CHARISMA);
    ...
    //::  Activate Cooldown
    SetTimelock(OBJECT_SELF, FloatToInt(TurnsToSeconds(10)), "Dragon Breath", 60);
}
