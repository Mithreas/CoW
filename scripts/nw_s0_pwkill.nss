...
// End of Spell Cast Hook
      ...
      if (GetIsObjectValid(oTarget))
      {
            if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
            {
                ...
                {
                      if (GetCurrentHitPoints(oTarget) < 251)
                          nDC++;
                      if (GetCurrentHitPoints(oTarget) < 201)
                          nDC++;
                      if (GetCurrentHitPoints(oTarget) < 151)
                          nDC++;
                      // New Arelith functionality: fort save or die
                      if(!MyResistSpell(OBJECT_SELF, oTarget) && !MySavingThrow(SAVING_THROW_FORT, oTarget, nDC, SAVING_THROW_TYPE_DEATH))
                      {
                          //Apply the death effect and the VFX impact
                          ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectDeath(), oTarget);
                          ApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, oTarget);
