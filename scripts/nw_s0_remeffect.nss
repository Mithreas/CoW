...
// End of Spell Cast Hook
    ...
    if(nEffect2 != 0)
    {
        RemoveBadEffect(nEffect2, oTarget);
    }
    if(nEffect3 != 0)
    {
        RemoveBadEffect(nEffect3, oTarget);
    }
    ApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, oTarget);
    if(nEffect1 == EFFECT_TYPE_DISEASE || nEffect2 == EFFECT_TYPE_DISEASE || nEffect3 == EFFECT_TYPE_DISEASE)
    {
        RemoveTaggedEffects(oTarget, EFFECT_TAG_DISEASE);
    }
    if(nEffect1 == EFFECT_TYPE_POISON || nEffect2 == EFFECT_TYPE_POISON || nEffect3 == EFFECT_TYPE_POISON)
    {
...
void RemoveBadEffect(int nEffectTypeID, object oTarget)
{
    ...
    while (GetIsEffectValid(eAOE))
    {
        if (GetEffectType(eAOE) == nEffectTypeID)
        {
            ...
            bValid = TRUE;
            if (!sepREGetIsDeathEffect(eAOE, oTarget))
            {
