    ...
    if (GetIsObjectValid(oTarget) == TRUE)
    {
        ...
        if (nTouch > 0)
        {
            ...
            if (nDamage > 0)
            {
                ...
                if (MySavingThrow(SAVING_THROW_FORT, oTarget, 20 + GetLevelByClass(CLASS_TYPE_ARCANE_ARCHER)) == 0)
                {
        ...
        IncrementRemainingFeatUses(OBJECT_SELF, FEAT_PRESTIGE_ARROW_OF_DEATH);
    }
}
