                       ...
                       int    nMultiplier = 0)
{
  ...
  if (GetIsKensai(oSpotter))
  {
     // Kensai spot bonus does not apply to disguise rolls
     nSpot = nSpot - 10;
  }
...
void UnDisguisePC(object oPC)
{
    ...
    // addition Dunshine:
    // is the disguiser as slave/prisoner? Then make sure this info stays in the dynamic name
    if (gvd_CheckIsClamped(oPC) == TRUE) {
...
void DisguisePC(object oPC, string sName = "")
{
    ...
    // addition Dunshine:
    // is the disguiser as slave/prisoner?
    if (gvd_CheckIsClamped(oPC) == TRUE) {

      // check if disguise skill is high enough, to hide the slave clamp
