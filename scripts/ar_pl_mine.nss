...
void main()
{
    ...
    //string sTemplate = GetResourceTemplate(OBJECT_SELF);
    string sTemplate = GetLocalString(OBJECT_SELF, ORE_TEMPLATE);
    if (sTemplate == "")                  return;
    int nChance      = GetMiningChanceByOre(sTemplate);
    if (nChance < 1)                      return;
    ...
    //::  Always give at least this amount of resources
    int nCurrentOred = GetLocalInt(OBJECT_SELF, "AMOUNT_ORED");
    int nMinimumOre  = GetLocalInt(OBJECT_SELF, "MINIMUM");

    while (nDamage >= GS_DAMAGE)
    {
        int bGiveOre = FALSE;
        if ( nCurrentOred < nMinimumOre ) {
            bGiveOre = TRUE;
            nCurrentOred = nCurrentOred + 1;
            SetLocalInt(OBJECT_SELF, "AMOUNT_ORED", nCurrentOred);
        }

        if ( (nChance + Random(100) >= 100) || bGiveOre )
        {
    ...
    int nTotalDamage = nActualDamage + GetLocalInt(OBJECT_SELF, "GS_TOTAL_DAMAGE");
