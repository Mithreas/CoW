...
// Integration notes:
// - Needs a placeable object with an inventory with tag GS_SEQUENCER in a DM-only area
//   (used for caching 2da values).
// - the script init_iprops needs to be called on module load (or shortly thereafter). 
// - to control which properties are available, add a GS_IP_DISABLED column to each iprp_* 2da file.
//   No value means allowed, TRUE means disabled. 
...
int gsIPGetIsValid(object oItem, int nProperty)
{
    // Author note - we use 2das to control this (every property 2da 
    // has a GS_IP_DISABLED column added). 
...
void gsIPSetOwner(object oItem, object oPC)
{
  SetLocalString(oItem, "GS_OWNER", GetName(oPC));
}
//------------------------------------------------------------------------------
int gsIPGetMaterialType(object oItem)
{
